...
bool nfa::try_merge_redundant_states()
{
	...
	const auto equal_after_rename = [&](std::size_t state_id0, std::size_t state_id1)
	{
		if(states_[state_id0].is_final!=states_[state_id1].is_final)
			return false;
		
		std::array state_copies{states_[state_id0], states_[state_id1]};

		for(auto& state: state_copies)
		{
			for(auto& [_,transitions]: state.transitions)
			{
				if(transitions.contains(state_id0))
				{
					transitions.erase(state_id0);
					transitions.insert(state_id1);
				}
			}
		}

		return state_copies[0]==state_copies[1];
	};
	...
	for(std::size_t state0_id=0;state0_id<states_.size();++state0_id)
	{
		...
		{
			if(states_[state0_id]==states_[state1_id] || equal_after_rename(state0_id,state1_id))
			{
